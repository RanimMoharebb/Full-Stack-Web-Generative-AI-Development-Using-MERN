        ...
        $jsonSchema: {
            ...
            properties: {
                ...
                age: {
                    bsonType: "int",
                    minimum: 18,
                    description: "'age' must be an integer >= 18 and is required"
                },
                department: {
                    enum: ["HR", "Engineering", "Finance"],
                    description: "'department' must be one of HR, Engineering, Finance and is required"
                }
            }
        }
    }
...
db.validEmployee.insertMany([
  { name: "Mohamed Ahmed", age: 25, department: "HR" },
  { name: "Noha Mahmoud", age: 30, department: "Engineering" },
  { name: "Malak Mohamed", age: 35, department: "Finance" },
  { name: "Mazen Mohamed", age: 45, department: "HR" },
  { name: "Eman Ali", age: 55, department: "Engineering" }
])

db.validEmployee.find({})
...
use Demo
var data = [
  { _id: 1, name: "Ranim Mohareb", age: 24, address: "Cairo", status: ["active", "verified"] },
  { _id: 2, name: "Ali Ahmed", age: 30, address: "Alexandria", status: ["inactive"] },
  { _id: 3, name: "Noha Mahmoud", age: 28, address: "Giza", status: ["active"] }
]
...
db.trainningCenter2.find({ age: 28 }).explain("executionStats")
// Before index: scanning type will be COLLSCAN (collection scan)
...
db.trainningCenter2.find({ age: 28 }).explain("executionStats")
// After index: scanning type will be IXSCAN (index scan)
...
db.trainningCenter2.find({ firstName: "Ranim", lastName: "Mohareb" }).explain("executionStats")
// Will show COLLSCAN (collection scan)


db.trainningCenter2.createIndex({ firstName: 1, lastName: 1 }, { name: "compound" })
// This tells MongoDB to use an index on firstName + lastName to speed up queries

//b.	Try find().explain after create index and mention scanning type
db.trainningCenter2.find({ firstName: "Ranim", lastName: "Mohareb" }).explain("executionStats")
// Will show IXSCAN (index scan)
// Now MongoDB will use the index instead of scanning the whole collection, making the query much faster.
...
// download the ziped file -> https://www.mongodb.com/try/download/database-tools
// run dump.exe
// a folder of the database and collection will be created i the same location
// in cmd -> "M:\Desktop\ITI\ITI\Technical Courses\10)) MongoDB\Day 3\mongodb-database-tools-windows-x86_64-100.14.1\bin\mongodump.exe" --db Demo --out "C:\Backup\DemoBackup"
...
// in cmd ->   "M:\Desktop\ITI\ITI\Technical Courses\10)) MongoDB\Day 3\mongodb-database-tools-windows-x86_64-100.14.1\bin\mongorestore.exe" --nsFrom="Demo.*" --nsTo="ITI_Course.*" "C:\Backup\DemoBackup\Demo"
use ITI_Course
